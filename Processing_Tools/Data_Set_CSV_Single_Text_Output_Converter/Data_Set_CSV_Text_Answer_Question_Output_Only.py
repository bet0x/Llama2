...
def write_cells_to_text_files(csv_file_path, output_folder):
    ...
    # Read the CSV file and extract the "Answer" and "Question" columns
    try:
        with open(csv_file_path, 'r', newline='', encoding='utf-8') as csvfile:
            reader = csv.DictReader(csvfile)
            for idx, row in enumerate(reader, start=1):
                question_content = row.get('Question', '')
                answer_content = row.get('Answer', '')

                # Create a text file with the format: "output_folder/question_answer_<index>.txt"
                file_path = os.path.join(output_folder, f"question_answer_{idx}.txt")

                try:
                    with open(file_path, 'w', encoding='utf-8') as file:
                        # Write the "Question" and "Answer" cell content to the text file
                        file.write(f"Question: {question_content}\n")
                        file.write(f"Answer: {answer_content}\n")
                except Exception as e:
                    print(f"Error writing to file for row {idx}: {e}")
